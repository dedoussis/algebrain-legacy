...
public class MainGUI extends JFrame {
	...
	private void createEvents() {
	    ...
	    actions.put(5, new AbstractAction() {
	        ...
	        public void actionPerformed(ActionEvent e) {
	        	...
	        	if (!tempHist.isEmpty()) txtrActiveStrTxtArea.setText(NodeFunctions.expression(tempHist.pop()));
	        	else tempHist = (Stack<AbstractNode>) history.clone();
		...
		mntmNewTransformationWizard.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(null, transFrame.getContentPane(), "Transformations", JOptionPane.PLAIN_MESSAGE);
					...
					for (Rule i : selectTr.getRules()){
						if (NodeFunctions.isOperator(i.getLhs())) {
							...
							if (!iOpe.getKey().matches("[\\+\\-\\/\\*\\^]|sin|cos|tan|log|ln|e")){
								if (!qualifiers.contains(iOpe.getKey())) qualifiers.add(iOpe.getKey());
							}
						}
					}
						...
						if (firstLhs.getKey().matches(selectTr.toString()) && firstLhs.getChildren().size()>1){
							...
							for (Component comp : paramPanel.getComponents()){
								if (comp instanceof JTextField) {
									...
									newWorkspace.getChildren().add(NodeFunctions.cannonical(NodeFunctions.expressionToNode(((JTextField) comp).getText())));		
								}
	...
	private void initComponents() {
		...
		transformationPanel.setBorder(new TitledBorder(null, "Transformation", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		gl_transformationPanel.setHorizontalGroup(
			gl_transformationPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_transformationPanel.createSequentialGroup()
					.addContainerGap()
					.addGroup(gl_transformationPanel.createParallelGroup(Alignment.TRAILING)
						.addComponent(transCmbBox, 0, 816, Short.MAX_VALUE)
						.addGroup(gl_transformationPanel.createSequentialGroup()
							.addComponent(btnAddOpe)
							.addPreferredGap(ComponentPlacement.RELATED)
							.addComponent(btnTransform)))
					.addContainerGap())
		);
		gl_transformationPanel.setVerticalGroup(
			gl_transformationPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_transformationPanel.createSequentialGroup()
					.addContainerGap()
					.addComponent(transCmbBox, GroupLayout.PREFERRED_SIZE, 26, GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
					.addGroup(gl_transformationPanel.createParallelGroup(Alignment.BASELINE)
						.addComponent(btnAddOpe)
						.addComponent(btnTransform))
					.addContainerGap())
		);
		...
		gl_terminalPanel.setHorizontalGroup(
			gl_terminalPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_terminalPanel.createSequentialGroup()
					.addContainerGap()
					.addGroup(gl_terminalPanel.createParallelGroup(Alignment.LEADING)
						.addGroup(gl_terminalPanel.createSequentialGroup()
							.addComponent(activeTxtPane, GroupLayout.DEFAULT_SIZE, 816, Short.MAX_VALUE)
							.addContainerGap())
						.addGroup(Alignment.TRAILING, gl_terminalPanel.createSequentialGroup()
							.addComponent(historyScrollPane, GroupLayout.DEFAULT_SIZE, 816, Short.MAX_VALUE)
							.addContainerGap())
						.addGroup(Alignment.TRAILING, gl_terminalPanel.createSequentialGroup()
		...
		gl_terminalPanel.setVerticalGroup(
			gl_terminalPanel.createParallelGroup(Alignment.TRAILING)
				.addGroup(gl_terminalPanel.createSequentialGroup()
					...
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(activeTxtPane, GroupLayout.PREFERRED_SIZE, 51, GroupLayout.PREFERRED_SIZE)
					.addGap(18)
	...
	private void submit(AbstractNode workspace, boolean sys) throws CloneNotSupportedException, IOException{
		...
		String manipulator;
		if (sys) manipulator = "Sys";
		else manipulator = "You";
		
		txtpnHistorytxtarea.setText(txtpnHistorytxtarea.getText() + "["+timeStamp+"] "+ manipulator +": "+ workspace + "\n");
	...
	public void export(){
		try {
			...
			writer.close();
			txtpnHistorytxtarea.setText(txtpnHistorytxtarea.getText() + "["+timeStamp.split(",")[1]+"] Sys: CAS_Terminal_[" + timeStamp + "].txt file exported sucessfully! \n");
	...
	public void setup() throws IOException{
		...
		((DefaultComboBoxModel<Transformation>) model).addElement(simpl);
			...
			Object frame = null;
			JOptionPane.showMessageDialog((Component) frame,
			"Preset transformations have already been created.",
		    "Load Setup Error",
		    JOptionPane.ERROR_MESSAGE);
		}
